...
I recently tried out [gganimate](https://doi.org/10.1101/206052) to make some dynamic plots in R. Today I want to try out two other packages; [gifski](https://CRAN.R-project.org/package=gifski) and [magick](https://cran.r-project.org/package=magick), to see how easy it is to export some simple GIFs. It looks like magick can do *a lot* of image processing tasks, while gifski is pretty focused on the one task: images > GIF.
...
save_gif(expr = pwalk(list(seq(0,10,0.1)), plot_sine),
         gif_file = "~/Downloads/gifski_sine_wave.gif",
...
dir.create("~/Downloads/sine_wave_images")

# Create images
walk(1:101, function(i) ggsave(filename = str_glue('~/Downloads/sine_wave_images/{str_pad(i,3,"left","0")}.png'),
                              plot = plot_sine(seq(0,10,0.1)[i]),
                              width = 6,height = 3, units = "in", dpi=72))

# Read in images, stitch them together, animate and write out
lapply(list.files("~/Downloads/sine_wave_images/", full.names = TRUE),
       image_read) %>% 
  image_join() %>%
  image_animate(fps = 10) %>%
  image_write(path = "~/Downloads/magick_sine_wave.gif")
```
...
gifski(png_files = list.files("~/Downloads/sine_wave_images/", full.names = TRUE),
       gif_file = "~/Downloads/gifski_sine_wave2.gif", 
       width = 72*6, height = 72*3,
       delay = 0.1, loop = TRUE, progress = TRUE)

# Remove individual images now we have our gifs
unlink("~/Downloads/sine_wave_images/", recursive = TRUE)
```

![](images/gifski_sine_wave2.gif)

Check the size of the new GIF
`utils:::format.object_size(file.size("~/Downloads/gifski_sine_wave2.gif"), units = "auto")`
